...
The Android client presents one of three outcomes for every recognition attempt: an accepted phrase, which is displayed and spoken; an unrecognised result, when the classifier identifies the input as not being one of the six phrases; or a low-confidence result, when the top class is a phrase but the model will not commit to it. Only the first produces speech. A row of indicators below the camera preview reports what the recogniser observed for each frame --- whether pose and hands were detected, the sustained frame rate, the per-frame conversion and extraction costs, and whether any frames were dropped --- so a failure during use can be attributed instead of guessed at.

\fillin{Insert client screenshots here: the accepted, unrecognised and low-confidence result states, and the telemetry indicator row during an active recording. The server configuration screen is optional, since it is reachable only after switching out of the default on-device mode.}
