...
\noindent Recording is started and stopped from the keyboard, so one clip corresponds to one deliberate performance, as in the recorder of Appendix~\ref{app:recorder}. Displaying the top three classes rather than only the winner was deliberate: a confident wrong answer and a genuine near-tie look identical when only the winning label is shown, and the two call for different responses.

\section{Android Client}
\label{app:ui-client}

The client presents one of three outcomes for every attempt: an accepted phrase, which is displayed and spoken; an unrecognised result, when the classifier positively identifies the input as not being one of the six phrases; or a low-confidence result, when the top class is a phrase but the model will not commit to it. Only the first produces speech. Figure~\ref{fig:ui-client} shows two accepted results, one from each of two phrase categories.
