...
The Android client presents one of three outcomes for every recognition attempt: an accepted phrase, which is displayed and spoken; a low-confidence result, which is reported without being spoken; and an unknown result, produced when the rejection gate refuses the input. A row of indicators below the camera preview reports what the server observed for each frame, so a failure during use can be attributed instead of guessed at.

\fillin{Insert client screenshots here: the accepted, low-confidence, and unknown result states, the telemetry indicator row, and the server configuration screen.}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|p{7.3cm}|}
        ...
        \hline
        Server & \texttt{result}    & Status, phrase, distance, and the three most probable classes \\
        \hline
