...
\noindent The network has 192{,}007 trainable parameters, which is small enough to train on a laptop CPU in minutes. The layer sizes above are not asserted. Section~\ref{sec:ablation} compares this architecture against five variants of it, under the evaluation protocol of Section~\ref{sec:archcompare}.
...
target, where the whole exported model is 881 kilobytes and runs without a GPU.
...
The six candidates vary one aspect of the design at a time: a smaller network (32/16/16 units), a larger one (128/64/64), a single recurrent layer instead of two, the removal of the dense head, and dropout raised from 0.3 to 0.5. The sixth is the shipped architecture itself, reconstructed to the same specification, so that it is measured on equal footing with the alternatives instead of being compared against a figure obtained some other way. Every candidate uses bidirectional LSTM cells; the comparison is over the size and arrangement of the layers, not over the choice of recurrent unit.
