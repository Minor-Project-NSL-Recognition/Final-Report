...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{6cm}|l|l|}
        \hline
        \textbf{\#} & \textbf{English phrase} & \textbf{Category} & \textbf{Class key} \\
        \hline
        1 & I can't breathe            & Medical           & \texttt{cant\_breathe} \\
        \hline
        2 & The building is on fire    & Fire              & \texttt{building\_on\_fire} \\
        \hline
        3 & Call the police            & Crime             & \texttt{call\_police} \\
        \hline
        4 & I need an ambulance        & Medical           & \texttt{need\_ambulance} \\
        \hline
        5 & Help me / I am in danger   & Generic emergency & \texttt{help\_danger} \\
        \hline
        6 & I need to go to the toilet & Basic need        & \texttt{need\_toilet} \\
        \hline
...
The \texttt{none} class holds deliberate non-examples: resting, idle movement, gestures outside the target set, and partial or abandoned signs. Giving the model explicit negatives lets it learn a decision boundary around the six real phrases instead of partitioning the whole input space among them. A second, independent rejection mechanism is described in Section~\ref{sec:oodgate}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|}
        ...
        \hline
        Dense 32, ReLU                  & embedding used by the rejection gate \\
        \hline
...
The 32-unit dense layer does double duty. It is the penultimate feature layer of the classifier, and it is also the space in which the open-set rejection gate measures distance. This is why the exported model has two outputs, not one.
...
\subsection{Fitting the Open-Set Rejection Gate}
\label{sec:oodgate}

After the deployable model is trained, the rejection gate of Section~2.4.8 is fitted on the same data. Every clip is passed through the network to obtain its 32-dimensional embedding. One prototype is computed per class as the mean embedding of that class, and a single covariance matrix shared across classes is estimated from the within-class deviations using Ledoit--Wolf shrinkage \cite{ledoit2004shrinkage}. The rejection threshold is set at the 99th percentile of the resulting in-distribution Mahalanobis distances, which for the committed model is 15.34. By construction, one per cent of genuine training clips fall beyond it.

The gate is then checked against out-of-distribution inputs, which is harder than it sounds because no dataset of things-that-are-not-NSL-emergency-phrases exists. Three hundred synthetic inputs are fabricated from the real clips instead. Half are time-shuffled, with the frame order randomised so the poses are individually valid but the motion is destroyed. The other half are cross-class splices, taking the first half of one phrase and the second half of a different one, which produces a clip that is locally plausible everywhere and globally not a sign. The separation between genuine and synthetic distances is measured as an AUROC.

Synthetic negatives are a weaker test than real ones. They establish that the gate responds to destroyed temporal structure, which is not the same as establishing that it rejects arbitrary human motion, and this limitation is revisited in Chapter~\ref{ch:conclusion}.
...
Three outcomes are possible, and the order of the checks matters:

\begin{enumerate}
    \item If the Mahalanobis distance to the nearest class prototype exceeds the threshold, the result is \texttt{unknown}. Nothing is displayed as a recognised phrase and nothing is spoken.
    \item Otherwise, if the softmax confidence reaches 0.75, the result is \texttt{accepted}. The phrase is displayed and spoken.
    \item Otherwise the result is \texttt{low\_confidence}. The user is told the input was not recognised clearly, and nothing is spoken.
\end{enumerate}

\noindent The rejection gate is checked before confidence, so a clip that the softmax assigns to one phrase with high probability is still refused if its embedding sits far from every prototype. The confidence threshold was lowered from the 0.85 proposed to 0.75 once the gate was in place, since the gate handles the failure mode the higher threshold was guarding against, and 0.85 was rejecting genuine signs. The best guess and the top three probabilities are returned in every case, including refusals, which is what makes a failure diagnosable during a demonstration.
