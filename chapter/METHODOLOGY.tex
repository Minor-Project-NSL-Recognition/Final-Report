...
The Android client captures camera frames, converts them, runs the recogniser, and speaks the result. In the fallback server mode it encodes and transmits frames instead of recognising them.
...
\noindent Only \texttt{accepted} speaks. The two refusals are distinguished because they call for different responses from the user --- \texttt{unknown} means the system saw motion it does not treat as a target phrase, while \texttt{low\_confidence} means it saw something phrase-like and would not commit --- but neither produces audio, so a wrong announcement and no announcement stay on the same footing.
