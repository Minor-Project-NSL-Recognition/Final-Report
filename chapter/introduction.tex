...
\noindent This project applies that combination to a narrow, high-value problem: recognising six NSL emergency phrases in real time and delivering each as on-screen text and speech. The six phrases are \textit{I can't breathe}, \textit{The building is on fire}, \textit{Call the police}, \textit{I need an ambulance}, \textit{Help me / I am in danger}, and \textit{I need to go to the toilet}. They span medical, fire, crime, and basic-need situations. Their signed form was established with Deaf experts before any data was recorded, so what the system recognises is real NSL and not an invention of the authors.
...
\begin{itemize}
    \item Most hearing people, including first responders and bystanders, do not understand Nepali Sign Language.
    \item Human interpreters are scarce and are almost never present when an emergency begins.
    \item Existing sign language recognition systems are built for high-resource sign languages. No system or dataset exists for NSL emergency communication \cite{sunuwar2024nsl23, shrestha2024nsl, poudel2025nsl}.
\end{itemize}

\noindent A deaf person in an emergency may therefore be unable to convey an urgent need to the people around them. What is missing is an accessible, low-cost tool that recognises essential NSL emergency phrases as they are signed and presents them in a form a hearing person understands immediately.
...
\begin{itemize}
    \item To establish the natural signed form of each of the six target phrases with NSL experts, so that the recorded data reflects how the phrases are actually signed.
    \item To build a custom dataset of NSL emergency phrases across multiple signers and recording conditions, since no suitable public dataset exists.
    \item To develop a landmark-based recognition pipeline using MediaPipe Holistic, representing each frame as 225 pose and hand coordinates.
    \item To normalise landmarks against anatomical anchors and standardise every clip to a fixed length derived from the recorded data.
    \item To train a bidirectional LSTM classifier and evaluate it on signers absent from its training data.
    \item To deploy the trained model in a real-time application that displays the recognised phrase and speaks it aloud, and to withhold output when the system is not confident.
\end{itemize}
...
\noindent The seventh class deserves a note, because it is not in the original proposal. A classifier trained on six phrases will assign one of those six to any input it receives, including someone scratching their head. For a system whose output is an emergency announcement, that behaviour is unacceptable. The negative class gives the model examples of what is not a sign, and a second rejection mechanism described in Chapter~\ref{ch:methodology} handles inputs unlike anything in training.
