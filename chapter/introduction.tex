...
In an emergency, a deaf person in Nepal has no reliable way to make an urgent need understood:

\begin{itemize}
    \item Bystanders and first responders do not understand Nepali Sign Language, and an interpreter is almost never present when an emergency begins.
    \item No recognition system or dataset exists for NSL emergency phrases; published work stops at the fingerspelling alphabet and static letters \cite{sunuwar2024nsl23, shrestha2024nsl, poudel2025nsl}.
\end{itemize}
...
The objective was to build a system that recognises six Nepali Sign Language emergency phrases from a live camera and speaks them aloud in Nepali. Specifically:

\begin{itemize}
    \item To record a custom dataset of the six phrases, with their signed form established by Deaf experts, since no such dataset exists.
    \item To train a landmark-based recogniser (MediaPipe Holistic and a bidirectional LSTM) and test it on signers it has never seen.
    \item To deploy it as a mobile application that runs offline on the device, displays the recognised phrase and speaks it in Nepali, and stays silent when it is not confident.
\end{itemize}
