...
\begin{longtable}{|p{3.4cm}|p{9.6cm}|}
        ...
        \hline
        MediaPipe Holistic & Pretrained pose and hand tracking. Produces the 225 landmark values per frame, offline during recording and again on the fallback server at inference. \\
        \hline
        ...
        \hline
        FastAPI and Uvicorn & The fallback inference service and its WebSocket endpoint. \\
        \hline
        ...
        \hline
        Kotlin & The Android bridge that runs the two landmark models and assembles the 225-vector on the device. \\
        \hline
...
\begin{enumerate}
    ...
    \item \textbf{Rejection of unknown input.} The system shall be trained with an explicit negative class covering non-signing motion, so that an input unlike any of the six target phrases has a class to be assigned to instead of being forced into one of them, and shall report such an input as unrecognised rather than announcing it.
    ...
    \item \textbf{Text and speech output.} On an accepted recognition the system shall display the phrase on screen in English and Nepali and play the recorded Nepali phrase aloud, using audio bundled with the application so that output does not depend on a voice being installed on the device.
...
\begin{figure}[H]
    ...
    \draw[dline, dashed, thick] (pre1) -- node[sloped, above, font=\sffamily\scriptsize, text=dgrey]
        {one definition, pinned by fixtures} (pre2);

    \end{tikzpicture}%
    }
    \caption{System block diagram. The offline path builds the model; the phone reproduces the same preprocessing and runs the classifier; the server offers the same path over a network as a fallback. The two shaded boxes are the same preprocessing definition.}
...
\begin{figure}[H]
    ...
    \draw[ret] (11.4,-5.5) -- node[above] {225 landmark values} (3.4,-5.5);
...
\begin{figure}[H]
    ...
    \caption{Class and module diagram. Solid arrows are use relationships; dashed arrows are imports or verification relationships. The heavier box on the right is the client's port of the Python preprocessing contract, pinned to it by generated fixtures.}
