...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3.4cm}|p{9.6cm}|}
        ...
        \hline
        scikit-learn & Stratified splits, class weights, the confusion matrix and classification report, and the Ledoit--Wolf covariance estimate used by the rejection gate. \\
        \hline
...
\begin{enumerate}
    ...
    \item \textbf{Rejection of unknown input.} The system shall measure the distance from the input to each class prototype in the model's embedding space and report the input as unknown when it lies beyond the fitted threshold, irrespective of the classifier's confidence.
...
\begin{figure}[H]
    ...
    \node[box]   (len)  at (6.6, 0)  {Frame-count analysis\\ \scriptsize p95 $\rightarrow$ 137};
    \node[shared](pre1) at (9.6, 0)  {Normalise\\ + standardise};
    \node[box]   (trn)  at (12.6, 0) {Train BiLSTM\\ \scriptsize + fit reject gate};
    \node[store] (art)  at (15.6, 0) {\texttt{model.tflite}\\ \scriptsize + gate statistics};
    ...
    \node[box]   (inf)  at (12.6, -3.4) {TFLite\\ forward pass\\ \scriptsize probs + embedding};
    \node[box]   (dcs)  at (15.6, -3.4) {Decision\\ \scriptsize accept / low / unknown};
...
\begin{figure}[H]
    ...
    % ---- bottom row ----
    \node[proc] (p4)  at (4.3, -4.8)  {\textbf{4}\\ Classify\\ and gate};
    \node[proc] (p5)  at (9.0, -4.8)  {\textbf{5}\\ Present\\ result};
    \node[ent]  (hear) at (13.7, -4.8) {Hearing recipient};
    \node[stor] (d2)  at (18.0, -4.8) {D2\\ model and gate statistics};
    ...
    % ---- row transition ----
    \draw[arr] (p3.south) -- (13.7,-2.4) -- node[above] {$137 \times 225$ tensor} (4.3,-2.4) -- (p4.north);

    \draw[arr] (p4) -- node[above] {status, phrase} (p5);
    \draw[arr] (p5) -- node[above] {text, speech} (hear);
    \draw[arr] (d2.south) -- (18.0,-6.6) -- node[below] {weights, prototypes, threshold} (4.3,-6.6) -- (p4.south);
    \end{tikzpicture}%
    }
...
\begin{figure}[H]
    ...
    \draw[ret] (15,-9.8) -- node[above] {status, phrase, distance, top three} (7.4,-9.8);
...
\begin{figure}[H]
    ...
    \node[cls] (od) at (0,-10.5) {%
        \textbf{ood}
        \nodepart{two}
        fit\_prototypes() \\
        mahalanobis\_min() \\
        save\_stats(), load\_stats()};
    ...
    \draw[dep] (od.east) -- (pr.south west);
...
Figure~\ref{fig:activity} shows the control flow of one recognition attempt, including the two decision points that can end it without output.
...
\begin{figure}[H]
    ...
    \node[act]  (a4)  at (0,-10.5)  {Normalise, standardise\\ to 137 frames};
    \node[act]  (a5)  at (0,-12.0)  {Forward pass:\\ probabilities, embedding};
    \node[dia]  (d3)  at (0,-14.0)  {distance within\\ threshold?};
    \node[dia]  (d4)  at (0,-16.4)  {confidence $\geq 0.75$?};
    \node[act]  (a6)  at (0,-18.3)  {Display phrase\\ and speak it};
    \node[act]  (a7)  at (5.8,-14.0){Report unknown,\\ stay silent};
    \node[act]  (a8)  at (5.8,-16.4){Report low confidence,\\ stay silent};
    \node[term] (en)  at (0,-19.9)  {End};
    ...
    \draw[arr] (a4) -- (a5);
    \draw[arr] (a5) -- (d3);
    \draw[arr] (d3) -- node[right] {yes} (d4);
    \draw[arr] (d3) -- node[above] {no} (a7);
    \draw[arr] (d4) -- node[right] {yes} (a6);
    \draw[arr] (d4) -- node[above] {no} (a8);
    \draw[arr] (a6) -- (en);
    \draw[arr] (a7.south) -- (a8.north);
