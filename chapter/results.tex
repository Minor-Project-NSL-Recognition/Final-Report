...
The application was built and installed on a physical device, and recognition works end to end with the network disabled: the user taps to begin, performs a phrase, taps to finish, and the recognised phrase appears and is spoken. The client reports its own costs as it runs, so the quantities in Table~\ref{tab:clientmeasurements} are read from the device rather than estimated.
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
        ...
        \textbf{Quantity} & \textbf{Value} \\
        \midrule
        Test device & OnePlus CPH2569, Qualcomm SM7325 \\
...
The work proceeded in eight increments, each delivering a module that could be exercised before the next began. Table~\ref{tab:sprints} records what each produced and what it changed about the plan, since several increments revised decisions made earlier.
