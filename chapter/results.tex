...
\mustdo{%
Every number and figure in this chapter comes from \texttt{results/metrics.json},
\texttt{results/model\_meta.json} and \texttt{data/processed/}, at
\textsc{seq\_len}~=~146 over 840 clips, and the four generated figures have been
refreshed from that same run. One defect in the source data is worth knowing about.

\texttt{results/metrics.json} carries a malformed \texttt{confusion\_matrix}: the
\texttt{call\_police} row sums to 121 against a support of 120, and the
\texttt{cant\_breathe} row has nine columns instead of seven. Neither can come
from the code that writes the file, so it has been hand edited at some point.
Table~\ref{tab:confusion} was therefore reconstructed from the per-class
precision and recall figures, which are self-consistent and pin every column sum,
and it agrees cell for cell with Figure~\ref{fig:confusion}, which was rendered
directly from the run. Do not patch the JSON --- re-run the pipeline and take the
matrix from the fresh output.

On the final retrain, run in this order --- \texttt{find\_seq\_len.py},
\texttt{build\_dataset.py}, \texttt{train\_eval.py}, \texttt{train\_model.py},
\texttt{export\_tflite.py} --- then copy the three model assets into
\texttt{app/assets/models/} and re-run the client test suite, or the phone will
carry a model that does not match this chapter. Then copy the four regenerated
figures into \texttt{img/} and update every number here, in the abstract, and in
Chapter~6.}
...
The \texttt{none} row is the one departure from the otherwise regular pattern. Signer 05 recorded no negatives and signer 06 recorded twice the usual number, so the class total still comes to 120 while those two signers contribute 60 and 80 clips overall rather than 70 each. This matters for one thing only, and it is worth stating before the fold results: signer 05's held-out fold is scored on six classes, not seven, because that signer has no \texttt{none} clips to be tested on. Since \texttt{none} is the hardest class, that fold is measured on a slightly easier problem than the other seven.
...
Frame counts across the 840 clips range from 30 to 217, with a median of 89 and a mean of 91.7. Figure~\ref{fig:seqlen} shows the distribution. The 95th percentile falls at 146 frames, which became the fixed sequence length.
...
\noindent At a capture rate of 15.7 frames per second, 146 frames corresponds to about 9.3 seconds of signing, and the median clip of 89 frames to about 5.7 seconds. Table~\ref{tab:stdmodes} shows how the standardisation step treated the clips.
...
Every fold converged and stopped on the early-stopping criterion before exhausting the 200-epoch budget. Table~\ref{tab:folds} reports where each fold stopped alongside its accuracy. The spread from 29 to 45 epochs reflects how quickly validation loss stopped improving on each training subset.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        signer01 & 140 & 0.9500 & 44 \\
        signer02 & 140 & 0.9786 & 43 \\
        signer03 & 140 & 0.9929 & 39 \\
        signer04 & 140 & 0.9643 & 29 \\
        signer05 & 60  & 0.9833 & 30 \\
        signer06 & 80  & 0.9000 & 36 \\
        signer07 & 70  & 0.9000 & 34 \\
        signer08 & 70  & 0.9714 & 45 \\
        \midrule
        \multicolumn{2}{l}{\textbf{Mean across folds}}      & \textbf{0.9551} $\pm$ 0.0340 & \\
        \multicolumn{2}{l}{\textbf{Pooled over 840 clips}}  & \textbf{0.9595} & \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent Mean accuracy across folds is 95.51\% with a standard deviation of 3.40 percentage points. Pooling all 840 predictions, each made by a model that had not seen its signer, gives 95.95\%.

The spread between folds is more informative than the mean. The best fold reaches 99.29\% and the two worst sit at 90.00\%, a range of 9.3 points. Since each fold differs only in which signer was withheld, that spread is a direct measurement of how much recognition quality depends on the individual. Reporting the mean alone would hide this; a system deployed to the public will meet signers at both ends of that range.

Two of the folds need a caveat rather than a straight reading. Signer 05 scores 98.33\%, but on 60 clips that carry no \texttt{none} examples, so that fold never faces the hardest class; it is the least comparable number in the table. Signer 06 scores 90.00\% on 80 clips, of which 20 are negatives rather than the usual 10, so that fold faces the hardest class twice as often as most. The two effects push in opposite directions and both trace back to the same irregularity in Table~\ref{tab:composition}. Signer 07, at the same 90.00\% on a conventional 70 clips, is the more honest worst case.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        \texttt{building\_on\_fire} & 0.9500 & 0.9500 & 0.9500 & 120 \\
        \texttt{call\_police}       & 0.9836 & 1.0000 & 0.9917 & 120 \\
        \texttt{cant\_breathe}      & 0.9520 & 0.9917 & 0.9714 & 120 \\
        \texttt{help\_danger}       & 0.9672 & 0.9833 & 0.9752 & 120 \\
        \texttt{need\_ambulance}    & 0.9524 & 1.0000 & 0.9756 & 120 \\
        \texttt{need\_toilet}       & 0.9833 & 0.9833 & 0.9833 & 120 \\
        \texttt{none}               & 0.9238 & 0.8083 & 0.8622 & 120 \\
        \midrule
        \textbf{Macro average}      & 0.9589 & 0.9595 & 0.9585 & 840 \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent The six emergency phrases achieve F1 between 0.950 and 0.992. Two of them, \texttt{call\_police} and \texttt{need\_ambulance}, are never missed at all: every one of their 240 clips was recognised, and what costs them precision is other classes being read as them rather than any failure of their own. The negative class is clearly the weakest at 0.862, with a recall of 0.808.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|ccccccc}
        ...
        \midrule
        \texttt{building\_on\_fire} & \textbf{114} & 0 & 0 & 0 & 0 & 0 & 6 \\
        \texttt{call\_police}       & 0 & \textbf{120} & 0 & 0 & 0 & 0 & 0 \\
        \texttt{cant\_breathe}      & 0 & 1 & \textbf{119} & 0 & 0 & 0 & 0 \\
        \texttt{help\_danger}       & 0 & 0 & 0 & \textbf{118} & 0 & 1 & 1 \\
        \texttt{need\_ambulance}    & 0 & 0 & 0 & 0 & \textbf{120} & 0 & 0 \\
        \texttt{need\_toilet}       & 0 & 0 & 0 & 1 & 0 & \textbf{118} & 1 \\
        \texttt{none}               & 6 & 1 & 6 & 3 & 6 & 1 & \textbf{97} \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent Thirty-four of the 840 predictions are wrong, and they fall into three groups that carry very different consequences.

Twenty-three errors are non-signs predicted as emergency phrases, in the bottom row. These are the most serious errors the system can make, because each one is an occasion where the system would announce an emergency that nobody signed, and they account for two thirds of all the errors in the matrix. They are spread across all six phrases, which suggests the cause is a genuine resemblance between some non-signing motion and the opening of a phrase rather than one specific confusable pair. This single row is where the system's remaining weakness lives, and Section~\ref{sec:discussion} returns to it.

Eight errors are emergency phrases predicted as \texttt{none}, in the last column: six for \texttt{building\_on\_fire}, one for \texttt{help\_danger}, and one for \texttt{need\_toilet}. Here the system stays silent when the user did sign. This leaves the user without help, though it does not produce a false alarm, and in a real deployment the user would notice the absence of a response and sign again.

Three errors are one emergency phrase confused with another, and each is isolated: one \texttt{cant\_breathe} read as \texttt{call\_police}, one \texttt{help\_danger} as \texttt{need\_toilet}, and one \texttt{need\_toilet} as \texttt{help\_danger}. A wrong phrase is worse than silence, because it sends the wrong help, so it is worth recording that this category is now the smallest of the three and contains no repeated pair. Confusion between the six phrases themselves is close to solved on this dataset; distinguishing them from everything that is not a sign is not.

Notably, no phrase is confused with any other in more than one clip, and \texttt{call\_police} and \texttt{need\_ambulance} are never missed at all. Every error in their columns is a non-sign or another phrase arriving from elsewhere.

\section{What the Random Split Shows}
\label{sec:leakage}

Under a stratified 70/15/15 random split, the same architecture and the same training recipe reach 98.41\% accuracy on 126 held-out clips. This is 2.5 percentage points above the pooled signer-independent figure, and 2.9 points above the mean across folds.

That gap is the point of reporting the number. Under a random split, clips from the same person appear in both training and test, so the model can score well by recognising the individual as much as the sign. The signer-independent protocol removes that possibility, and the accuracy falls. The 98.41\% figure describes how well the system recognises phrases performed by people it has already been trained on, which is not the situation any real user is in.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Random split, stratified 70/15/15 & 0.9841 & performance on known signers \\
        Signer-independent, mean of 8 folds & 0.9551 & performance on a new signer \\
        Signer-independent, pooled & 0.9595 & performance on a new signer \\
        \bottomrule
    \end{tabular}
\end{table}
...
First, no fold ran to the epoch limit. Early stopping halted every fold between 29 and 45
...
different signer, produce accuracies between 0.9000 and 0.9929 with a standard deviation
of 3.40 percentage points. A model that had memorised its training signers would be
expected to collapse on some folds and not others; the worst fold here is nine points below
the best, not thirty. The spread is nonetheless wider than the differences between any two
architectures tested in Section~\ref{sec:ablation}, which is the finding this chapter keeps
returning to.

Fourth, and least comfortably, the random-split comparison puts a number on the
memorisation that does remain. The 2.5-point gap between 98.41\% and 95.95\% is the
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccccc}
        ...
        \textbf{current}       & 64/32/32   & LSTM & 0.3 & 192{,}007 & 0.961 & [0.944, 0.978] & 42 \\
        single\_layer          & 64/--/32   & LSTM & 0.3 & 152{,}839 & 0.960 & [0.939, 0.980] & 43 \\
        no\_dense\_head        & 64/32/--   & LSTM & 0.3 & 190{,}151 & 0.952 & [0.930, 0.974] & 40 \\
        high\_dropout          & 64/32/32   & LSTM & 0.5 & 192{,}007 & 0.947 & [0.925, 0.970] & 50 \\
...
through this sweep, and it reports mean 0.961 with a standard deviation of 0.024, against
0.9551 and 0.0340 from \texttt{results/metrics.json} in Table~\ref{tab:folds}. The two use
the same protocol, seed, splits, callbacks and sequence length, so they ought to agree, and
they differ by 0.6 points.

The difference is the runner, not the model. The sweep is executed by
\texttt{scripts/sweep\_parallel.py}, which runs each candidate in a separate process with
four intra-op threads, while \texttt{train\_eval.py} runs single-process. Deterministic
operations are enabled in both, but the thread count changes the order of parallel
reductions --- the same effect Section~\ref{sec:experimentsetup} already measures at up to
2.7 points.

Before submission, either re-run the sweep through \texttt{scripts/sweep\_architectures.py}
single-process so the two figures are produced identically, or state in the text above that
the sweep column was produced by a different runner and is comparable within itself but not
against Table~\ref{tab:folds}. Then delete this box.}
...
An accuracy near 96\% on a first attempt at an under-resourced sign language invites
...
reflects this directly: across the twenty-one possible pairs of phrases, no pair is confused
more than once. Second, the landmark representation discards background, lighting, and appearance
...
most practice performing it. Their four folds average 0.9714 against 0.9387 for the other
four, a gap of 3.3 points. Four folds a side is far too few to call this conclusive, and the
signer 05 and signer 06 irregularities noted above sit on the weaker side of the comparison,
but the direction is the one the explanation predicts and the size of it is no longer
negligible. The headline figure is an accuracy on a population half of which built the
system.

None of this makes 95.95\% wrong. It makes it an accuracy on this task as specified:
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
        ...
        Sustained landmarking rate while signing & \fillin{fps} \\
        Per-frame conversion cost & \fillin{ms} \\
        Per-frame landmark extraction cost & \fillin{ms} \\
        Corrections, normalisation and forward pass, per clip & \fillin{ms} \\
...
The \texttt{none} class is already the weakest at 0.862 F1, and it degrades as the
...
Every accuracy in this chapter rests on eight people, four of whom are the authors. The standard deviation across folds is 3.40 points and the range is 9.3, which means the estimate of performance on a new signer is itself uncertain. A ninth signer with an unusual signing style could plausibly score below any fold reported here.
...
The \texttt{none} class carries a recall of 0.808, and twenty-three non-signs were predicted as emergency phrases. In deployment these are the errors that produce a false announcement, which is the most serious failure mode this system can have. They are also two thirds of every error in the matrix: confusion among the six phrases has almost disappeared, and what remains is the boundary between the phrases and everything that is not one.

That single row therefore accounts for the difference between the system's accuracy and the accuracy it could have. If every non-sign were correctly rejected, pooled accuracy would rise from 95.95\% to 98.7\%, and the six phrases would already be at that level.

Reducing this error is a question of what the negative class covers rather than of the threshold it is checked against. The \texttt{none} class holds 120 clips describing the complement of six phrases --- an unbounded space sampled 120 times --- and it is the only class whose examples are not a specific repeatable performance. Widening it with more varied non-signing motion, particularly the kind of motion these twenty-three clips resemble, is the direct way to give the classifier a better boundary to place, and it is the single most useful addition to this evaluation. How many of the twenty-three cleared the confidence threshold is not recorded in the saved metrics and would need measuring separately; that number decides whether a stricter threshold would help at all, and it should be obtained before any threshold is changed.
